...
async function runProtectFlow() {
  console.log('🛡️  [Bento Guard] Starting Simplified SDK Protect Flow...');
  ...
  console.log(`   * Loaded Agent Wallet Address: ${agentAddress}`);

  // 2. Initialize the Bento SDK
  const client = BentoGuardClient.initialize({
    agentWalletPrivateKey: agentKey,
    endpoint: process.env.BENTO_API_URL || 'http://localhost:4001', // Target local port
  ...
  try {
    ...
    console.log('🔑 Signing instruction with Agent\'s private key...');
    ...
    // 5. Submit protection request using the singular protect() function
